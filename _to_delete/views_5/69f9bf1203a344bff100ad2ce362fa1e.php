...
<title>Pay <?php echo e($order->number); ?> — SmartEPT by Ametecs</title>
<style>
...
<div class="card">
  <div class="head"><div class="mk">EPT</div>
    <b>SmartEPT — <?php echo e($order->quote_number ? 'Quotation ' . $order->quote_number : 'Order ' . $order->number); ?></b>
    <span><?php echo e($order->tenant->company_name); ?> · <?php echo e($order->description); ?><?php if($order->status === 'quote'): ?> · awaiting management payment <?php endif; ?></span>
  </div>
  <div class="body">
    <?php $__currentLoopData = $order->line_items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $line): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <div class="ln"><span><?php echo e($line['description']); ?></span><b><?php echo e($order->currency === 'INR' ? '₹' : '$'); ?><?php echo e(number_format($line['amount'], 2)); ?></b></div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    <div class="ln"><span>GST</span><b><?php echo e($order->currency === 'INR' ? '₹' : '$'); ?><?php echo e(number_format($order->tax_amount, 2)); ?></b></div>
    <div class="tot"><span>Total</span><span><?php echo e($order->currency === 'INR' ? '₹' : '$'); ?><?php echo e(number_format($order->total, 2)); ?></span></div>
    <?php if($received > 0.01 && $balance > 0.01): ?>
      
      <div class="ln"><span>Received so far</span><b style="color:#08875D">− <?php echo e($order->currency === 'INR' ? '₹' : '$'); ?><?php echo e(number_format($received, 2)); ?></b></div>
      <div class="tot" style="font-size:16px"><span>Balance payable<?php echo e($order->credit_due_date ? ' by ' . $order->credit_due_date->format('d M Y') : ''); ?></span><span><?php echo e($order->currency === 'INR' ? '₹' : '$'); ?><?php echo e(number_format($balance, 2)); ?></span></div>
    <?php endif; ?>

    <?php if($order->status === 'paid' || $balance <= 0.01 || request('paid')): ?>
    ...
    <?php else: ?>
      <?php if($razorpayEnabled): ?>
        <button class="paybtn" id="rzpBtn">Pay <?php echo e($received > 0.01 ? 'balance ' : ''); ?><?php echo e($order->currency === 'INR' ? '₹' : '$'); ?><?php echo e(number_format($balance, 2)); ?> — UPI / Card / NetBanking</button>
      <?php endif; ?>
      <?php if($stripeEnabled): ?>
        <a class="alt" href="/pay/<?php echo e($order->number); ?>/<?php echo e($token); ?>/stripe">Pay by international card (Stripe)</a>
      <?php endif; ?>
...
<?php if($razorpayEnabled && $order->status !== 'paid'): ?>
<script src="https://checkout.razorpay.com/v1/checkout.js"></script>
<script>
document.getElementById('rzpBtn').onclick = async function () {
  const res = await fetch('/pay/<?php echo e($order->number); ?>/<?php echo e($token); ?>/razorpay-order', {
    method: 'POST',
  ...
  const rzp = new Razorpay({
    ...
    description: '<?php echo e($order->number); ?>',
    prefill: {email: '<?php echo e($order->tenant->email); ?>', contact: '<?php echo e($order->tenant->phone); ?>'},
    theme: {color: '#0E7C8F'},
    handler: async function (resp) {
      const cb = await fetch('/pay/<?php echo e($order->number); ?>/<?php echo e($token); ?>/razorpay-callback', {
        method: 'POST',
...
<?php /**PATH C:\laragon\www\smartept-central\resources\views/checkout.blade.php ENDPATH**/ ?>
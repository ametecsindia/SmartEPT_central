...
function waEdit(t) {
  ...
  <textarea id="wa_body" rows="3" placeholder="Hi @0, thanks for your interest in SmartEPT…">${esc(p.body||'')}</textarea>
  <label>Sample values (comma-separated — used for the test send; must match the @0 variables)</label>
...
function auForm(u) {
  ...
  openModal('<h2>' + (isNew ? 'Add' : 'Edit') + ' admin user</h2>'
    ...
    + (isNew ? '<label>Email</label><input id="au-email" type="email" placeholder="name.com">' : '<div class="sub">' + esc(u.email) + '</div>')
...
async cms() {
  ...
  ${f('landing_contact_email','Contact email shown on the site','sales.com')}
  ${f('whatsapp_number','WhatsApp number (all buttons)','919000098877')}
  ${f('sales_email','Lead alerts go to (email)','sales.com')}</div></div>
...
async settings() {
  ...
  ${f('mail_from_address','From address (e.g. billing.com)')}</div>
...
function newCoupon() {
  ...
  <div><label>Exclusive to one email (blank = public)</label><input id="cf-excl" type="email" placeholder="cfo.in"></div>
  </div>

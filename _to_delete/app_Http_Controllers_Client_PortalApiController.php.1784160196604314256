...
class PortalApiController extends Controller
{
    ...
    {
        $rows = $this->tenant()->orders()->with('invoice:id,order_id,number')->latest()->get()
            ->map(function ($o) {
                return [
    ...
    public function quote(Request $request)
    {
        ...
        if (! empty($data['coupon_code'])) {
            [$coupon, $reason] = \App\Models\Coupon::check($data['coupon_code'], (int) $data['devices']);
    ...
    public function updateBillingProfile(Request $request)
    {
        ...
        if (! empty($data['gstin'])) {
            ...
            $data['state_code'] = ($data['state_code'] ?? null) ?: substr($data['gstin'], 0, 2);
        }

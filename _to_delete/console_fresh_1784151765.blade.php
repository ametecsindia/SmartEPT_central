...
// ---------- pages ----------
let PAGE = 'dashboard';
const TITLES = {dashboard:'Dashboard',tenants:'Clients / Tenants',trials:'Trials',licences:'Licences',
plans:'Plans & Pricing',orders:'Orders & Payments',invoices:'Invoices',storage:'Cloud Storage',
...
async function showTenant(id) {
  ...
  ${t.deployment==='cloud'?'SmartEPT-Managed Cloud':'Client-Hosted'} · ${esc(t.currency)} · Setup fee: ${t.setup_fee_paid?'<span class="gain">paid ✓</span>':'not yet charged'}</div>
...
function tenantForm(t = {}) {
  ...
  </div><label>Address</label><textarea id="f_addr" rows="2">${esc(t.address||'')}</textarea>
...
async function issueLicence() {
  ...
  <div><label>Billing</label><select id="il_billing"><option>annual</option><option>monthly</option></select></div>
...
async function newOrder() {
  ...
  <div><label>Billing</label><select id="no_billing" onchange="refreshQuote()"><option>annual</option><option>monthly</option></select></div>

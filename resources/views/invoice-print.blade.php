...
@php
    ...
    // Credit view (master prompt §10): received/balance computed from the
    // payments ledger; an unsettled invoice displays DUE with its due date.
    $received = $order ? $order->received() : (float) $invoice->total;
    $balance = $order ? $order->balance() : 0.0;
    $isDue = $invoice->status === 'issued' && $balance > 0.01;
    $statusLabel = $isDue ? 'DUE' : strtoupper($invoice->status);
@endphp
...
<div class="meta">
  ...
  <div class="box"><b>Invoice Details</b>
    ...
    @if ($isDue && $invoice->due_date)
    <div class="kv"><span>Payment due by</span><span style="font-weight:700;color:#B7791F">{{ $invoice->due_date->format('d M Y') }}</span></div>
    @endif</div>
</div>
...
<div class="blocks">
  <div class="bx"><b>Payment</b>
    ...
    @if ($order)
      ...
      @endif
      @if ($isDue)
      <div class="kv"><span>Received so far</span><span>{{ $sym }}{{ number_format($received, 2) }}</span></div>
      <div class="kv"><span>Balance payable</span><span style="font-weight:700;color:#B7791F">{{ $sym }}{{ number_format($balance, 2) }}</span></div>
